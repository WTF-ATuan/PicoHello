Shader "GGDog/Tunnel"
{
    Properties
    {
		...
		_FarColor ("FarColor", COLOR) = (0.5,0.5,0.5,1)
	...
	//	Blend SrcAlpha OneMinusSrcAlpha
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
				...
				half uv_dis = smoothstep(0,0.1,v.uv.xy.x*(1-v.uv.xy.x));

                o.vertex = UnityObjectToClipPos(v.vertex + 0.002*Noise*v.normal*uv_dis);
				...
				//漸層外圍天空色
				o.NdotV_FarFog.y = smoothstep(100,200,worldPos.z);
            ...
            half4 frag (v2f i) : SV_Target
            {
				...
				half4 Rim = 1-saturate(smoothstep(0,1,i.NdotV_FarFog.x));
				
				col.rgb = lerp(_ShadowColor,_Color,Rim);
				
				col.rgb = lerp(_FarColor,col,1-i.NdotV_FarFog.y);

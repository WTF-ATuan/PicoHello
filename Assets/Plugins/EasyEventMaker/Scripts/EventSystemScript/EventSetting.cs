...
public class EventSetting : MonoBehaviour {
	...
	public static bool[] globalBoolean = new bool[999];
	public static int[] globalInt = new int[999];
	...
	public void Activate(){
		...
		if(type == ParametersType.SpawnPrefab){
			SpawnPrefab(0);
		}
		...
		if(type == ParametersType.PlayLegacyAnimation){
			...
			/*if(t.GetComponent<Status>() && t.GetComponent<Status>().mainModel){
				t = t.GetComponent<Status>().mainModel;
			}*/
		...
		if(type == ParametersType.ChangeScene){
			...
			if(boolVar){
				mainPlayer.transform.position = vector3Var;
				mainPlayer.transform.eulerAngles = vector3Var2;
			}
		...
		if(type == ParametersType.MoveObject){
			Transform t = SetTarget();
			if(!t){
				NextEvent();
				return;
			...
			t.GetComponent<MovingLookingEvent>().destinationObject = null;
			t.GetComponent<MovingLookingEvent>().moveSpeed = floatVar;
			t.GetComponent<MovingLookingEvent>().destinationPosition = vector3Var;
	...
	void ShakeCamera(){
		if(!Camera.main.GetComponent<ShakeScreen>()){
			Camera.main.gameObject.AddComponent<ShakeScreen>();
		}
		Camera.main.GetComponent<ShakeScreen>().Shake(floatVar , floatVar2);
		NextEvent();
	}

	...
	SubShader
	{
		...
		{	
			...
			v2f vert (appdata v)
			{
				...
				half4 Noise1 = tex2Dlod(_NoiseTex, half4( scruv*_NoiseTex_ST.xy +_Time.y*(0.25,-0.25)*2 ,0,0))*smoothstep(-30,150,o.CameraDistance);
				half4 Noise2 = tex2Dlod(_NoiseTex, half4( scruv*_NoiseTex_ST.xy +_Time.y*(-0.25,0.15)*2 ,0,0))*smoothstep(-30,150,o.CameraDistance);
			...
			half4 frag (v2f i) : SV_Target
			{
			   ...
			   _Vector.z*= smoothstep(-150,150,i.CameraDistance);
				...
				half4 refrCol = tex2D(_BackTex0, scruv ) ;

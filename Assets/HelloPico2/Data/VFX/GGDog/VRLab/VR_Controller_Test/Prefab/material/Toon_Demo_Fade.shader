	...
	SubShader
	{
		...
		{	
			...
			half4 frag (v2f i) : SV_Target
			{
                ...
                half Dir2 = saturate(dot(i.normal_VS.xyz,_BackLightDir));

                half4 BackLightcol = lerp( 0 , 1 , step(Dir2,0.5) );

                col = lerp(saturate(col),BackLightcol,_BackLightLerp);

				half Near_Alpha = smoothstep(-5,1,i.normal_VS.w);

				return half4(col.rgb,Near_Alpha);

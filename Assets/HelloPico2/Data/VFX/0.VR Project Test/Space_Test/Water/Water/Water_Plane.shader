    ...
    SubShader 
    {
        ...
        Pass 
        {
            ...
            { 
				...
				fixed3 tangent_LightDir = normalize(i.lightDir);
				fixed3 tangent_ViewDir = normalize(i.viewDir);
                fixed3 tangent_HalfDir = normalize(tangent_LightDir*1.1 + tangent_ViewDir)/1.01;
                //fixed3 tangent_HalfDir = normalize(tangent_LightDir*1.1 + (tangent_ViewDir+0.15)/1.25);
				
				_NormalMap_ST.xy*=_NormalMapUVTiling;

				fixed4 packedNormal = tex2D(_NormalMap,_NormalMap_ST.xy*i.worldPos.xz/50 +float2(0.05,0.1)*_Time.y );
				fixed4 packedNormal2 = tex2D(_NormalMap,_NormalMap_ST.xy*i.worldPos.xz/50 +float2(0,0.1)*_Time.y );

				packedNormal = packedNormal/2+packedNormal2/2;

				fixed3 tangent_Normal = UnpackNormal(packedNormal);
				...
				//高光
                fixed spec = dot(tangent_Normal,tangent_HalfDir);

                //抗鋸齒處理
                fixed w = fwidth(spec) * _SpecularGradient;
                fixed3 specular = lerp(0,_LightColor0.rgb* _Specular.rgb, smoothstep(-w, w, spec +_SpecularScale - 1)) ;
				...
				return fixed4( lerp(ambient,diffuse,Color_lerp) + specular + spot, 1);
            }

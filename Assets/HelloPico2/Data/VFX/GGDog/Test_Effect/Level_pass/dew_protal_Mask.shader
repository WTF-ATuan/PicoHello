	...
	SubShader
	{
		...
		{	
			...
			v2f vert (appdata v)
			{
				...
				half cd = 1/o.CameraDistance*150;
				scruv = (scruv+0.5*(cd-1))/cd;

				half4 Noise1 = tex2Dlod(_NoiseTex, half4( scruv*_NoiseTex_ST.xy +_Time.y*(0.25,-0.25)*2 ,0,0))*smoothstep(0,150,o.CameraDistance);
				half4 Noise2 = tex2Dlod(_NoiseTex, half4( scruv*_NoiseTex_ST.xy +_Time.y*(-0.25,0.15)*2 ,0,0))*smoothstep(0,150,o.CameraDistance);
				...
				o.vertex = UnityObjectToClipPos(v.vertex + v.normal * pow((Noise1+Noise2+Noise3*smoothstep(0.5,1,uvdis))*uvdis/2,2) );
			...
			half4 frag (v2f i) : SV_Target
			{
			   ...
			   _Vector.z*= smoothstep(-150,150,i.CameraDistance);
				...
				half4 refrCol = tex2D(_BackTex, scruv + (1-Rim/i.CameraDistance*50)/50) ;

				refrCol = lerp(_ShadowColor,refrCol,(1-smoothstep(0,1000,i.CameraDistance)));

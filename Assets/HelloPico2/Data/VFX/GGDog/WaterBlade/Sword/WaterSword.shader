	...
	SubShader
	{
		...
		{	
			...
			v2f vert (appdata v)
			{
				...
				float Noise1 = snoise(v.vertex*20*float2(10,1)+_Time.y*(0,-0.35)*5);
				float Noise2 = snoise(v.vertex*10*float2(10,1)+_Time.y*(0,-0.15)*2);

				float noise = (Noise1*0.175+Noise2*0.15);

				o.vertex = UnityObjectToClipPos(v.vertex + v.normal * noise*noise);
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half CameraDistance950_1500 =  saturate(smoothstep(70,250,i.CameraDistance));
					// CameraDistance950_1500 *=  saturate(smoothstep(-10,-10,CameraDistance950_1500));

				_ShadowColor = lerp(_ShadowColor,1,CameraDistance950_1500);
				_RimColor = lerp(_ShadowColor,0,CameraDistance950_1500);
				_Gloss = lerp(_Gloss,1000,CameraDistance950_1500);
